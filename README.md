...
**3. Prediction & Live Tracking** (`predict_matches.py`, `log_prediction.py`)
...
`predictions_log.csv` contains real predictions logged before actual 2026 World Cup matches, along with the results once they finished. This is updated throughout the tournament — see the file for the current running accuracy.
...
python log_prediction.py log
python log_prediction.py update
```
